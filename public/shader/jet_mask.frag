...
void main(){
    ...
    vec2 tile = floor( (v_uv + vec2(u_time * 0.9, u_time * 0.65) ) * vec2(0.5));
    vec2 tileCoord = (v_uv + vec2(u_time * 0.9, u_time * 0.65)) * vec2(0.5) - tile;
    vec4 effectorTex = texture2D(u_effectorTex, tileCoord);
    float effectorVal = effectorTex.r * 1.1;
